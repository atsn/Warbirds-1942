...
public class Enemy extends AbstractGameObject
{
	...
	private void Ai(float deletatime)
	{
		if (MyType == Enemytype.blueEnemy)
		{
			...
			else
			{
				...
				velocity.setAngle(rotation - 90).nor().scl(3);
			}
		}
	...
	private int GetDirection()
	{
		...
		float wantetrotation = 180 - (float) Math.toDegrees(Math.atan2(level.player.position.x - position.x, level.player.position.y - position.y));

		float difrence = rotation - wantetrotation;

		if (Math.abs(difrence) > 180)
		{
			if (difrence >0)
			{
				difrence += -360;
